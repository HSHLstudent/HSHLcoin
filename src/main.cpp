...
#if defined(NDEBUG)
# error "HSHLcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x708c751d428ed01bffe5355f9f756876922e66f0c576ad043a3bd1082eef68fa");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // HSHLcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "HSHLcoin Signed Message:\n";
...
{
    // HSHLcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // HSHLcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // HSHLcoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // HSHLcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // HSHLcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // HSHLcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x4dc3650c0a6e6b55c379654790a41a1b8fd64d48dbe9adf21bc932b5e424fddc");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Spiegel 28/Mar/2018 Babyboom in Deutschland";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04a0bfd9cfde050271c92023b72040db6d9e0bed5acf65376cdd5618f751e21a09374b809abb9d957b94d1a606aaf141114d10725118c1e411a784b60fdeb5df4c") <<OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1522241043;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084836593;

        if (fTestNet)
        {
            block.nTime    = 1522241038;
            block.nNonce   = 386976322;
        }
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x49469b67694de998af632366f009e99d9e163005316e86991d0c2969c75a37f6"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // HSHLcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("HSHLcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("HSHLcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static HSHLcoinMiner(CWallet *pwallet)
{
    printf("HSHLcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("HSHLcoin-miner");
    ...
    try { loop {
        ...
        printf("Running HSHLcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("HSHLcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&HSHLcoinMiner, pwallet));
}
